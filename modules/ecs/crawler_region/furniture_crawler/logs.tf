...
moved {
  from = module.ecs.aws_cloudwatch_log_group.furniture_crawler_log_group
  to   = aws_cloudwatch_log_group.furniture_crawler_log_group
}
...
resource "aws_iam_role_policy" "ecs_logs_policy" {
  name = "${local.derived_ecs_task_execution_role_name}-ecs-furniture-crawler-logs"
