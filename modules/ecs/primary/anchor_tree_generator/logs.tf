...
resource "aws_cloudwatch_log_group" "anchor_tree_generator_log_group" {
  ...
  name              = "/aws/ecs/anchor_tree_generator"
  retention_in_days = 14
  tags = {
    Project = var.project
    Name    = "${var.project}-anchor-tree-generator-log-group"
  }
}
...
resource "aws_iam_role_policy" "ecs_logs_policy" {
  name = "${local.derived_ecs_task_execution_role_name}-ecs-anchor-tree-generator-logs"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/ecs/anchor_tree_generator:*"
      }
    ]
  })
}
