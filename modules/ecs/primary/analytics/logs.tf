...
resource "aws_cloudwatch_log_group" "analytics_log_group" {
  ...
  name              = "/aws/ecs/analytics"
  retention_in_days = 14
  tags = {
    Project = var.project
    Name    = "${var.project}-analytics-log-group"
  }
}
...
resource "aws_iam_role_policy" "ecs_logs_policy" {
  name = "${local.derived_ecs_task_execution_role_name}-ecs-analytics-logs"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/ecs/analytics:*"
      }
    ]
  })
}
