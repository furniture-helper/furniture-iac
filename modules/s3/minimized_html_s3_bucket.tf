resource "aws_s3_bucket" "minimized_html_storage" {
  ...
  bucket = "furniture-minimized-html"
  ...
  tags = {
    Project = var.project
    Name    = "${var.project}-minimized_html-bucket"
  }
}
...
output "minimized_html_storage_s3_bucket_arn" {
  value       = aws_s3_bucket.minimized_html_storage.arn
  description = "ARN of the S3 bucket used for minimized HTML storage"
}

output "minimized_html_storage_s3_bucket_name" {
  value       = aws_s3_bucket.minimized_html_storage.bucket
  description = "Name of the S3 bucket used for minimized HTML storage"
}
